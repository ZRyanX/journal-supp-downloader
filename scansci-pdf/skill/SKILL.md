...
| `scansci_pdf_download` | 下载单篇论文（完整参数控制） | `identifier`（必需，DOI 或 arXiv ID）、`strategy`（可选）、`use_vpnsci`（可选）、`use_tor`（可选）、`bibtex`（可选） |
...
| `scansci_pdf_camofox_login` | camofox 持久化浏览器登录（WebVPN/CARSI/EZProxy/自定义） | `login_type`（"webvpn"/"carsi"/"ezproxy"/"custom"）、`custom_url`（login_type=custom 时必需） |
| `scansci_pdf_camofox_status` | 检查 camofox-browser 运行状态 | 无 |
| `scansci_pdf_camofox_import_cookies` | 导入 Netscape 格式 cookie 文件到 camofox | `cookie_file`（必需，cookie 文件路径） |
| `scansci_pdf_import_browser_cookies` | 通过 camofox 浏览器捕获登录 cookie | `url`（默认 ScienceDirect）、`max_wait`（默认 300） |
...
4. 选择机构并完成 SSO 登录
5. 登录完成后关闭浏览器
6. Cookies 自动捕获并导入 camofox-browser，后续所有下载自动使用
