...
def _resolve_doi_url(doi: str) -> str | None:
    ...
    try:
        ...
        log.info(f"   [WebVPN] _resolve_doi_url failed: {e}")
    return None
...
def _try_vpnsci_camofox(doi: str, output_path: Path, config: dict[str, Any]) -> dict[str, Any] | None:
    ...
    try:
        ...
        _auth_url_signals = list(_stoks) + ["/do/off/ui/auth", "sign_in", "signin", "login"]

        log.info(f"   [WebVPN-Camofox] Page title: '{title}' URL: {url_now[:80]}")
        if "登录" in title or "身份" in title or "二次认证" in title or "CAS" in title or any(t in url_now.lower() for t in _auth_url_signals):
