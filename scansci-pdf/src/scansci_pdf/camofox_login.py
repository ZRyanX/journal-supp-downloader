    ...
    *,
    ...
    try:
        ...
        while elapsed < max_wait:
            ...
            curr_path = parsed_curr.path.lower()
            
            is_auth_page = (
                "login" in curr_path or 
                "cas" in curr_path or 
                "sso" in curr_path or 
                "sign_in" in curr_path or 
                "signin" in curr_path or
                "passport.escience.cn" in parsed_curr.netloc.lower()
            )
            if not is_auth_page:
                cookies = context.cookies()
                if len(cookies) > 3:
...
def webvpn_login(config: dict[str, Any]) -> bool:
    ...
    from urllib.parse import urlparse
    parsed_base = urlparse(base).hostname or ""

    def _detect(context: Any, page: Any) -> bool:
        try:
            curr = page.url.lower()
            from urllib.parse import urlparse
            parsed_curr = urlparse(page.url)
            curr_path = parsed_curr.path.lower()
            
            is_on_vpn = parsed_base.lower() in parsed_curr.netloc.lower()
            is_auth = any(x in curr_path for x in ("login", "cas", "sso", "sign_in", "signin", "oauth", "security/device", "auth/oauth"))
            cookies = context.cookies()
            return is_on_vpn and not is_auth and len(cookies) > 2
        except Exception:
            return False
